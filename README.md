...
When you run each script, it contains an internal testing block (`if __name__ == "__main__":`) that automatically tests the primitive. 

**To verify the result is correct:**
1. **Look at the Output Logs:** Each script will print out the input values, intermediate states, and final output.
2. **Check for "Success" or "PASS":** Many scripts print a confirmation message at the end (e.g., `Decryption matches original: True`, `Integrity Check: PASS`, `Tampered tag verification: FAILED (As Expected)`).
3. **No Assertion Errors:** All scripts are designed with self-tests. If an implementation is incorrect, Python will throw an `AssertionError` or crash. If the script finishes executing successfully without errors and returns you to the prompt, the implementation logic is mathematically and programmatically correct!
